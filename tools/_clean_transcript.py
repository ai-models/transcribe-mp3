...
def handle_whisper(string):
    og_string = string
    override_cases = {
        'MITD 5.1': "emmeyeteedee five point one",
        '9-11': 'nine-eleven',
        'M-I-T-D 5-1': "emmeyeteedee five one",
        '952': 'nine five two',
    ...
    if any(char.isdigit() for char in string):
        ...
        return code, response
    if re.search(r'(\w+\.)+(com|net|org|gov)', string)\
            or re.search(r'[A-Z]{2,}', string):
        code = 500
        # print('##### String contains numbers')
        response = "String contains numbers"
        return code, response
    ...
    filtered_string = ''.join([c for c in string if c in allowed_characters])
