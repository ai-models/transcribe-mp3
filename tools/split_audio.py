...
def convert_mp3_to_wav(input_dir, output_dir, segment_time, output_bitrate=16000):
    ...
    for mp3_file in os.listdir(input_dir):
        if mp3_file.endswith('.mp3'):
            ...
            subprocess.run(['ffmpeg', '-i', mp3_path, '-ar', {output_bitrate}, '-ac', '1', '-sample_fmt', 's16', '-f', 'segment', '-segment_time', str(segment_time), '-c:a', 'pcm_s16le', output_template], check=True)
